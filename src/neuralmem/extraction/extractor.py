...
import re
import logging
from neuralmem.core.types import Entity, Relation, Memory, MemoryType
...
class MemoryExtractor:
    ...
    def _infer_type(self, text: str) -> MemoryType:
        text_lower = text.lower()
        if any(w in text_lower for w in ['yesterday', 'today', 'ago', '昨天', '今天', '之前', 'happened']):
    ...
    def _score_importance(self, text: str, entities: list[Entity]) -> float:
        ...
        if any(w in text.lower() for w in ['important', 'critical', 'must', '重要', '关键', '必须']):
            score += 0.2
