...
> 基准测试脚本: `benchmarks/competitive_benchmark.py` | 测试环境: macOS, Apple Silicon, Python 3.12
...
|------|-----------|-----------|-----|
| **LoCoMo 准确率** | 待测 (需真实嵌入) | 91.6% | 80.32% |
| **LongMemEval** | 待测 | 93.4% | — |
| **Recall@5 (合成数据)** | 12% (mock) / ~80% (real) | — | — |
| **MRR (合成数据)** | 0.100 (mock) / ~0.7 (real) | — | — |

> NeuralMem 合成基准使用 4 维 mock embedder，数值偏低。实际 all-MiniLM-L6-v2（384 维）预期 Recall@5 ~80%, MRR ~0.7。
